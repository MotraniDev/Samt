...
| **App settings** | The user's preferences hub: display language, theme package, update check, adhkar reminders, Hijri/calendar preferences (day offset, calendar country override, special-day reminder master/sets/time), and official community links; distinct from diagnostics, which shows technical process status only. |
...
| **Calendar day sheet** | Per-day surface opened from a Hijri calendar cell: dual dates, special-day labels with source, and optional mute for that day's special-day reminder. Not a prayer-times view and not a free-form notes editor. |
...
Cloud sync, Store listing, mosque search, paid Google Maps, live web scrape of Adhkar, full Hisn-level / fatwa library, bundled Adhkar recitation audio (reader UI may prepare for it), free-form theme builder, silent auto-install of updates, claiming official prayer times, official moon-sighting calendar authority, multi-country holiday CMS, free-form personal calendar notes (v1 special-day reminders only).
